...
@router.get("/api/check/connectivity", tags=["check"], description="Check connectivity to example.com")
def check_connectivity(logger: Logger = Depends(get_cloud_logger)):
    try:
        ...
        return 200, f"Connectivity check: Got response {response.status_code} from http://example.com"
